...
def list_banks():  # Liste toutes les banques supportées par Bridge
...
def list_user_accounts(user_uuid: str):  # Liste comptes bancaires utilisateur Bridge
    connector = BankingConnectorService()
    ...
    accounts = connector.get_user_accounts(user_uuid)
...
def import_transactions(
    ...
    try:
        result = connector.import_transactions_to_db(
        ...
        return {
            **result,
            "message": f"{result['imported']} transactions importées avec succès"
        }
...
def sync_account(request: SyncAccountRequest):  # Déclenche synchronisation compte bancaire
    connector = BankingConnectorService()
    ...
    success = connector.sync_account(request.account_id)
    ...
    return {
        "success": True,
        "message": f"Synchronisation du compte {request.account_id} déclenchée"
    }


@router.post("/items", response_model=CreateItemResponse)
def create_item(request: CreateItemRequest):  # Crée item Bridge pour connexion banque
    connector = BankingConnectorService()
...
def get_account_transactions(
    ...
    transactions = connector.get_account_transactions(
